...
def train(model, device, train_loader, optimizer):
    ...
    for batch_idx, data in enumerate(train_loader):
        ...
        for ii in range(len(outputs)):
            output = outputs[ii]
            loss_train = loss_fn(output, data.y.view(-1, 1).float().to(device))
        loss_train.backward()
        optimizer.step()

def test(model, drug_feas, target_feas, labels, idx_test, device):
    model.eval()
    # features = features.to(device)
    drug_feas = drug_feas[idx_test, :]
    target_feas = target_feas[idx_test, :]
    labels = labels[idx_test]
    drug_feas = drug_feas.to(device)
    target_feas = target_feas.to(device)
    labels = labels.to(device)
    with torch.no_grad():
        outputs = model(drug_feas, target_feas)
